...
const props = defineProps<{ certificate: any }>();

const form = useForm({
    microchip: props.certificate.microchip || '',
    registration_number: props.certificate.registration_number || '',
    name: props.certificate.name || '',
    breed: props.certificate.breed || '',
    variety: props.certificate.variety || '',
    sex: props.certificate.sex || 'MACHO',
    color: props.certificate.color || '',
    birth_place: props.certificate.birth_place || '',
    birth_date: props.certificate.birth_date?.split('T')[0] || '',
    image: props.certificate.image || '',
    valid: props.certificate.valid ?? true,
    certificate_number: props.certificate.certificate_number || '',
    certificate_date: props.certificate.certificate_date?.split('T')[0] || '',
    breeder: props.certificate.breeder || '',
    owner: props.certificate.owner || '',
    co_owner: props.certificate.co_owner || '',
    address: props.certificate.address || '',
    zip_code: props.certificate.zip_code || '',
    neighborhood: props.certificate.neighborhood || '',
    city: props.certificate.city || '',
    phone: props.certificate.phone || '',
    titles: props.certificate.titles || [],
    observations: props.certificate.observations || '',
    status: props.certificate.status || 'VIGENTE',
    is_active: props.certificate.is_active ?? true,
});

const submit = () => {
    form.put(`/cms/certificates/${props.certificate.id}`);
};

const breadcrumbs = [
    { title: 'Dashboard', href: '/dashboard' },
    { title: 'Certificados', href: '/cms/certificates' },
    { title: 'Editar', href: `/cms/certificates/${props.certificate.id}/edit` },
];
</script>

<template>
    <Head :title="`Editar: ${certificate.name} - CMS`" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 p-4">
            <div class="flex items-center justify-between">
                <h2 class="text-2xl font-bold tracking-tight">Editar Certificado</h2>
            </div>
                ...
                <div class="grid gap-6 lg:grid-cols-3">
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        <Card>
                            ...
                            <CardContent class="space-y-4">
                                <JsonEditor v-model="form.titles" label="Títulos" :rows="4" />
