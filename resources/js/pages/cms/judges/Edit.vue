...
const props = defineProps<{ judge: any }>();

const form = useForm({
    name: props.judge.name || '',
    email: props.judge.email || '',
    phone: props.judge.phone || '',
    city: props.judge.city || '',
    photo: props.judge.photo || '',
    license_type: props.judge.license_type || '',
    categories: props.judge.categories || [],
    breeds: props.judge.breeds || [],
    description: props.judge.description || '',
    is_active: props.judge.is_active ?? true,
    sort_order: props.judge.sort_order || 0,
});

const submit = () => {
    form.put(`/cms/judges/${props.judge.id}`);
};

const breadcrumbs = [
    { title: 'Dashboard', href: '/dashboard' },
    { title: 'Jueces', href: '/cms/judges' },
    { title: 'Editar', href: `/cms/judges/${props.judge.id}/edit` },
];
</script>

<template>
    <Head :title="`Editar: ${judge.name} - CMS`" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 p-4">
            <div class="flex items-center justify-between">
                <h2 class="text-2xl font-bold tracking-tight">Editar Juez</h2>
            </div>
                            ...
                            <CardContent class="space-y-4">
                                ...
                                <div class="grid gap-4 sm:grid-cols-2">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <Input id="license_type" v-model="form.license_type" />
                                    </div>
                                </div>
